...
你要直接站在其中一个选择上，轻轻推用户一把，给用户一个方向。
不要中立。
不要“两边都可以”。
不要“看你自己”。
你像一个温柔、懂情绪的朋友，会给用户一种：
“她已经帮我做了决定”的感觉。
你必须真正帮用户做决定。
不要总说：“你已经有答案了”“其实你心里知道”
不要敷衍。不要给模棱两可答案。
${modePrompt}
要求：
1. 回答自然
...
app.post("/api/wall/posts", async (req, res) => {
  try {
    ...
    if (content.length > 200) return res.status(400).json({ error: "内容太长啦" });
...
`,
    ...
    const aiReply = completion.choices[0].message.content || "别急呀，也许答案会慢慢浮现。";
    ...
    await pool.query(
      ...
      VALUES ($1, $2, $3, $4, $5)
      `,
      [newPost.id, newPost.content, newPost.reply, newPost.likes, newPost.createdAt]
    );
...
// =========================
app.get("/", (req, res) => {
  res.json({ ok: true, message: "择光 AI server running ✨" });
});
